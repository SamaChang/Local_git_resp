	...
	,spk1_thread(nullptr), isCircleShoot(false), all_stream_thread(nullptr), isCut(false),multiple(1), cap_rgb_ir_spk_thread(nullptr)
{
	...
	this->setWindowTitle("Capture/Preview_Tool_V1.41");
...
static void GetFrame(Camera *camera, BYTE *buff, LONG len)
{
	...
	if (val == 0xc)
	{
		...
		QImage img = QImage((unsigned char*)rgb.data, rgb.cols, rgb.rows, rgb.step, QImage::Format_RGB888);
		QPixmap pixmap = QPixmap::fromImage(img);
		cap_tool->mainThread_show(cap_tool->widgets.at(0), pixmap);
	}
	if (val == 0xa)//ir&spk
	{
		...
		QImage img_ir = QImage((unsigned char*)ir_src.data, ir_src.cols, ir_src.rows, ir_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		QPixmap pixmap_ir = QPixmap::fromImage(img_ir);
		cap_tool->mainThread_show(cap_tool->widgets.at(1), pixmap_ir);
		//spk
		Mat spk_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream_2[0]*/buff + UVC_IMG_HEIGHT * UVC_IMG_WIDTH);
		QImage img_spk = QImage((unsigned char*)spk_src.data, spk_src.cols, spk_src.rows, spk_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		QPixmap pixmap_spk = QPixmap::fromImage(img_spk);
		cap_tool->mainThread_show(cap_tool->widgets.at(2), pixmap_spk);
	}
}
...
void CaptureTool::playAllVideo()
{
	...
	bool bret = camera->Open(/*string2wstring("smxUSBD Video Device")*/dev_name_string);
	if (!bret)
	{
		QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Camera Open Error!"), QMessageBox::Ok);
		//return;
...
void CaptureTool::m_cap()
{
	static uint32_t rgb_ir_dep_pic_id = 0;
	int count;
	char a = format_value[0];
	char b = format_value[1];
	char c = format_value[2];
	do
	{
		if (delay_time_to_shoot)
			Sleep(delay_time_to_shoot * 1000);
		...
		if (format_value[0])
		{
			QString rgbName = saveFileName + "/" + "RGB" + QString::number(rgb_ir_dep_pic_id) + "_640x360.yuv";
			QFile fileRGB(rgbName);
			if (!fileRGB.open(QIODevice::WriteOnly))
			{
				QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
...
void CaptureTool::keyPressEvent(QKeyEvent *ev)
{
	if (ev->key() == Qt::Key_P)
	{
		m_cap();

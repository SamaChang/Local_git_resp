...
bool CCameraDS::OpenCamera(int nCamID, bool bDisplayProperties, int nWidth, int nHeight)
{
	...
	mt.subtype = MEDIASUBTYPE_YUY2;// MEDIASUBTYPE_RGB24;    原图
	//mt.subtype = MEDIASUBTYPE_RGB24;// MEDIASUBTYPE_YUY2;   yuv数据预览类似amcap
...
*/
static int l = 0;
IplImage* CCameraDS::QueryFrame(void* Buffer, long max_size)
{
	...
	m_pSampleGrabber->GetCurrentBuffer(&m_nBufferSize, (long*)Buffer);   //抓原图

	/****************************************/
	//std::ostringstream  cur;
	//cur << "./rgb" << l << ".raw";
	//std::string dd = cur.str();
	////std::ofstream cc("./aa.raw", std::ios::binary);//app表示每次操作前均定位到文件末尾
	//std::ofstream cc(dd, std::ios::binary);//app表示每次操作前均定位到文件末尾
	//if (cc.fail()) {
	//	std::cout << "error\n";
	//}
	//cc.write((char*)/*&yuyv[0]*/(Buffer/* + 640 * 400 * 2*/), 1280 * 720 * 2);

...
CaptureTool::CaptureTool(QWidget *parent)
	...
	,spk1_thread(nullptr), isCircleShoot(false), all_stream_thread(nullptr), isCut(false),multiple(1), cap_rgb_ir_spk_thread(nullptr)
{
    ui.setupUi(this);
	this->setWindowIcon(QIcon("./Logo.png"));
	this->setWindowTitle("Capture/Preview_Tool_V1.5");
	...
	if (names.empty() == true)
	{
		QMessageBox::warning(0, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("找不到任何摄像头设备!"), QMessageBox::Ok);
		return ;
	}
	int m_nCamCount = names.size();
	BOOL b_find_dev = false;
	for (int i = 0; i < m_nCamCount; i++)
	{
		cam_dev_name.push_back(names[i]);
		cam_dev_num.push_back(i);
		b_find_dev = TRUE;
	}

	if (!b_find_dev) {
		QMessageBox::warning(0, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("不能打开摄像头设备!"), QMessageBox::Ok);
		return;
	}
	//海康
	//initCamera();
...
{
	HWND hwnd = (HWND)(widgets.at(5))->winId();     //获取控件的windows窗口句柄
...
//}
//EVB
static void GetFrame(Camera *camera, BYTE *buff, LONG len)
{
	//rgb
	BYTE val = buff[0];
	if (val == 0xc)
	{
		memcpy(&(cap_tool->all_stream_1[0]), buff, UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);  //rgb
		Mat yuv_img(UVC_IMG_WIDTH, UVC_IMG_HEIGHT, CV_8UC2, buff);
		Mat rgb;
		cvtColor(yuv_img, rgb, CV_YUV2RGB_YUY2); //CV_YUV2BGR_YUY2
		transpose(rgb, rgb);
		flip(rgb, rgb, 0);//flip(srcCopy, srcCopy, 1);  //rotate 270 
		QImage img = QImage((unsigned char*)rgb.data, rgb.cols, rgb.rows, rgb.step, QImage::Format_RGB888);
		QPixmap pixmap = QPixmap::fromImage(img);
		QPixmap fitpixmap = pixmap.scaled(/*UVC_IMG_WIDTH*//*180*//*360*/270, /*UVC_IMG_HEIGHT*//*320*//*640*/480, Qt::KeepAspectRatio, Qt::SmoothTransformation); //按比例缩放
		cap_tool->mainThread_show(cap_tool->widgets.at(0), fitpixmap);
	}
	if (val == 0xa)//ir&spk
	{
		memcpy(&(cap_tool->all_stream[0]), buff, UVC_IMG_HEIGHT*UVC_IMG_WIDTH);  //ir
		memcpy(&(cap_tool->all_stream_2[0]), buff + UVC_IMG_HEIGHT * UVC_IMG_WIDTH, UVC_IMG_HEIGHT*UVC_IMG_WIDTH);   //spk
		//ir
		Mat ir_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream[0]*/buff);
		QImage img_ir = QImage((unsigned char*)ir_src.data, ir_src.cols, ir_src.rows, ir_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		QPixmap pixmap_ir = QPixmap::fromImage(img_ir);
		QPixmap fitpixmap_ir = pixmap_ir.scaled(/*UVC_IMG_WIDTH*//*180*//*360*/270, /*UVC_IMG_HEIGHT*//*320*//*640*/480, Qt::KeepAspectRatio, Qt::SmoothTransformation); //按比例缩放  
		cap_tool->mainThread_show(cap_tool->widgets.at(1), fitpixmap_ir);
		//spk
		Mat spk_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream_2[0]*/buff + UVC_IMG_HEIGHT * UVC_IMG_WIDTH);
		QImage img_spk = QImage((unsigned char*)spk_src.data, spk_src.cols, spk_src.rows, spk_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		QPixmap pixmap_spk = QPixmap::fromImage(img_spk);
		QPixmap fitpixmap = pixmap_spk.scaled(/*UVC_IMG_WIDTH*//*180*//*360*/270, /*UVC_IMG_HEIGHT*//*320*//*640*/480, Qt::KeepAspectRatio, Qt::SmoothTransformation); //按比例缩放  
		cap_tool->mainThread_show(cap_tool->widgets.at(2), fitpixmap);
	}
}
//门禁
static void GetFrame_2(Camera *camera, BYTE *buff, LONG len)
{
	//rgb
	BYTE val = buff[0];
	if (val == 0xc)
	{
		memcpy(&(cap_tool->all_stream_4[0]), buff, UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);  //rgb
		Mat yuv_img(UVC_IMG_WIDTH, UVC_IMG_HEIGHT, CV_8UC2, buff);
		Mat rgb;
		cvtColor(yuv_img, rgb, CV_YUV2RGB_YUY2); //CV_YUV2BGR_YUY2
		transpose(rgb, rgb);
		flip(rgb, rgb, 1);//flip(srcCopy, srcCopy, 0);  //rotate 90 
		QImage img = QImage((unsigned char*)rgb.data, rgb.cols, rgb.rows, rgb.step, QImage::Format_RGB888);
		QPixmap pixmap = QPixmap::fromImage(img);
		QPixmap fitpixmap = pixmap.scaled(/*UVC_IMG_WIDTH*//*180*//*360*/270, /*UVC_IMG_HEIGHT*//*320*//*640*/480, Qt::KeepAspectRatio, Qt::SmoothTransformation); //按比例缩放  
		cap_tool->mainThread_show(cap_tool->widgets.at(3), fitpixmap);
	}
	if (val == 0xa)//ir&spk
	{
		memcpy(&(cap_tool->all_stream_3[0]), buff, UVC_IMG_HEIGHT*UVC_IMG_WIDTH);  //ir
		//memcpy(&(cap_tool->all_stream_2[0]), buff + UVC_IMG_HEIGHT * UVC_IMG_WIDTH, UVC_IMG_HEIGHT*UVC_IMG_WIDTH);   //spk
		//ir
		Mat ir_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream[0]*/buff);
		QImage img_ir = QImage((unsigned char*)ir_src.data, ir_src.cols, ir_src.rows, ir_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		QPixmap pixmap_ir = QPixmap::fromImage(img_ir);
		QPixmap fitpixmap = pixmap_ir.scaled(/*UVC_IMG_WIDTH*//*180*//*360*/270, /*UVC_IMG_HEIGHT*//*320*//*640*/480, Qt::KeepAspectRatio, Qt::SmoothTransformation); //按比例缩放  
		cap_tool->mainThread_show(cap_tool->widgets.at(4), fitpixmap);
		//门禁无spk
		//Mat spk_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream_2[0]*/buff + UVC_IMG_HEIGHT * UVC_IMG_WIDTH);
		//QImage img_spk = QImage((unsigned char*)spk_src.data, spk_src.cols, spk_src.rows, spk_src.step, QImage::Format_Indexed8);  //ir原图转QImage
		//QPixmap pixmap_spk = QPixmap::fromImage(img_spk);
		//cap_tool->mainThread_show(cap_tool->widgets.at(4), pixmap_spk);
	}
}
...
void CaptureTool::playAllVideo()
{
	...
	all_stream.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH);  //ir
	//all_stream_1.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 3);   //rgb  yuv
	all_stream_1.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);   //rgb  yuv
	all_stream_2.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH /** 2*/);  //图漾。。depth   //通用。。spk
	//dg(门禁)
	all_stream_3.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH);  //ir
	all_stream_4.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);  //rgb
	//all_stream_5.resize(UVC_IMG_HEIGHT*UVC_IMG_WIDTH);  //spk

	//bool bret = cameraDs.OpenCamera(0, false, UVC_IMG_HEIGHT, UVC_IMG_WIDTH);    //当没有摄像头时，参数没有0，会报错。

	Camera * _camera = camera;
	_camera->SetCallBack([&_camera](double, BYTE* buff, LONG len) {GetFrame(_camera, buff, len); });
	dev_name_string = cam_dev_name.at(1);   //需加个判断，是否有摄像头设备
	//bool bret = camera->Open(/*string2wstring("smxUSBD Video Device")*/dev_name_string);//以设备名打开（如果两个设备重名，则都打开的是一个设备）
	bool bret = camera->Open(0);//以设备序号打开
	if (!bret)
	{
		QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Camera_EVB Open Error!"), QMessageBox::Ok);
		//return;
	}
	//else
	//{
	...
	Camera * _camera_2 = camera_2;
	_camera_2->SetCallBack([&_camera_2](double, BYTE* buff_2, LONG len_2) {GetFrame_2(_camera_2, buff_2, len_2); });
	dev_name_string = cam_dev_name.at(0);   //需加个判断，是否有摄像头设备
	//bool bret_2 = camera_2->Open(/*string2wstring("smxUSBD Video Device")*/dev_name_string);//以设备名打开（如果两个设备重名，则都打开的是一个设备）
	bool bret_2 = camera_2->Open(1);//以设备序号打开
	if (!bret_2)
	{
		QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Camera_DG Open Error!"), QMessageBox::Ok);
		//return;
	}
...
void CaptureTool::open_capture_set_form()
{
	...
	connect(capset, SIGNAL(pic_format(char *)), this, SLOT(format_set(char *)));
...
//获取选择抓图照片格式
void CaptureTool::format_set(char *value)
{
	//format_value = (char *)malloc(3 * (sizeof(char)));
	//format_value = new char[3];
	//format_value = value;
	memcpy(format_value, value, 6);
}
...
void CaptureTool::m_cap()
{
	static uint32_t rgb_ir_dep_pic_id = 0;
	int count;
	char a = format_value[0]; //rgb_evb
	char b = format_value[1]; //ir_evb
	char c = format_value[2]; //spk_evb
	char d = format_value[3]; //Camera_evb
	char e = format_value[4]; //camera_dg
	char f = format_value[5]; //camera_hk
	do
	{
		if (delay_time_to_shoot)
			Sleep(delay_time_to_shoot * 1000);
		...
		if (format_value[3])
		{
			...
			if (format_value[0])
			{
				QString rgbName = saveFileName + "/" + "RGB" + QString::number(rgb_ir_dep_pic_id) + "_640x360_evb.yuv";
				QFile fileRGB(rgbName);
				if (!fileRGB.open(QIODevice::WriteOnly))
				{
					QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
				...
				fileRGB.write((const char *)&all_stream_1[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);  //RGB
				fileRGB.close();
			}
			//ir
			if (format_value[1])
			{
				QString irName = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + "_360x640_evb.raw";
				QFile fileIR(irName);
				if (!fileIR.open(QIODevice::WriteOnly))
				{
					QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
				...
				fileIR.write((const char *)&all_stream[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);
				fileIR.close();
			}
			//spk
			if (format_value[2])
			{
				QString spkName = saveFileName + "/" + "SPK" + QString::number(rgb_ir_dep_pic_id) + "_360x640_evb.raw";
				QFile fileSpk(spkName);
				if (!fileSpk.open(QIODevice::WriteOnly))
				{
					QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
				...
				fileSpk.write((const char *)&all_stream_2[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);
				fileSpk.close();
			}
		}
		//dg(门禁)
		if (format_value[4])
		{
			...
			if (format_value[0])
			{
				QString rgbName = saveFileName + "/" + "RGB" + QString::number(rgb_ir_dep_pic_id) + "_640x360_dg.yuv";
				QFile fileRGB(rgbName);
				if (!fileRGB.open(QIODevice::WriteOnly))
				{
					QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
				...
				fileRGB.write((const char *)&all_stream_4[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);  //RGB
				fileRGB.close();
			}
			//ir
			if (format_value[1])
			{
				QString irName = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + "_360x640_dg.raw";
				QFile fileIR(irName);
				if (!fileIR.open(QIODevice::WriteOnly))
				{
					QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
				...
				fileIR.write((const char *)&all_stream_3[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);
				fileIR.close();
			}
		}
		//hk(海康)
		if (format_value[5])
		{
			QString savename = saveFileName + "/" + "HK" + QString::number(rgb_ir_dep_pic_id) + "_hk.bmp";
			string sana = savename.toStdString();
			if (!NET_DVR_SetCapturePictureMode(BMP_MODE))    //设置抓图模式，BMP模式，JPEG_MODE为JPEG模式
			{
				QMessageBox::warning(this, "error", "NET_DVR_SetCapturePictureMode error;error number " + QString::number(NET_DVR_GetLastError()));
			}
			if (!NET_DVR_CapturePicture(IRealPlayHandle, (char *)sana.c_str()))
			{
				QMessageBox::warning(this, "error", "NET_DVR_CapturePicture error;error number " + QString::number(NET_DVR_GetLastError()));
			}
		}
		rgb_ir_dep_pic_id++;
		count = rgb_ir_dep_pic_id;
		status->setText("Completed pictures:" + QString::number(count));
	} while (isCircleShoot && shot_times--);
	shot_times = record_times;
}
...
void CaptureTool::cap_rgb_ir_spk()
{
	cap_rgb_ir_spk_thread = new std::thread(&CaptureTool::m_cap, this);
}

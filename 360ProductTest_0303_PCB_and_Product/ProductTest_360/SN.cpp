...
void SN::on_pushButton_clicked()
{
	...
	writeCSV(snInf, NULL, "PASS", QString::number(0), currentTime, QString::fromLocal8Bit("写入SN/Mac"), NULL, NULL, NULL, NULL, NULL);
	this->close();
...
void SN::on_pushButton_3_clicked()
{
	...
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss");
	recordResult("FAIL");
	writeCSV(snInf, NULL, "FAIL", QString::number(0), currentTime, QString::fromLocal8Bit("写入SN/Mac"), NULL, NULL, NULL, NULL, NULL);
	this->close();
...
void SN::on_pushButton_2_clicked()
{
	...
	if (ba.length() == 14)
	{
		...
		dateY = reverse_uint32(dateYear);
		devN = reverse_uint32(devNum);
		match_sn_mac(mac_wifi, mac_bt, mac_eth);
	}
	//uint8_t dateYear[4] = { ba.at(6),ba.at(7),ba.at(8),ba.at(9) };
	//uint8_t devNum[4] = { ba.at(10),ba.at(11),ba.at(12),ba.at(13) };
	//uint32_t dateY = reverse_uint32(dateYear);
	...
	//Eth_mac *eth =new Eth_mac()/*= nullptr*/;
	Eth_mac eth;
	//pack_360_SN(5, 1, 'A', 'P', dateY, devN, /*(uint8_t *)*/&sn_360);
	pack_360_SN('5', '1', 'A', 'P', dateY, devN, /*(uint8_t *)*/&sn_360);
	pack_360_hwid('P', 1, 0, /*send_buf + sizeof(SN_360)*/&hwid);
	pack_360_wifi_mac(0xB4, 0xef, 0x1c, 0x01, mac_wifi[0], mac_wifi[1], &wifi);
	pack_360_bt_mac(0xB4, 0xef, 0x1c, 0x01, mac_bt[0], mac_bt[1], &bt);
	pcak_360_eth_mac(0xB4, 0xef, 0x1c, 0x01, mac_eth[0], mac_eth[1], &eth);
	pack_360_Info(send_buf + sizeof(send_360Protocol), 3, 4, 32, sn_360, hwid, wifi, bt, eth);
	uint32_t payload_len = sizeof(product_info);
	pack360_head(CMD_SN, payload_len, send_buf);
	...
	{
		if (ay.at(4) == 1)
			QMessageBox::information(nullptr, QString::fromLocal8Bit("通过"), QString::fromLocal8Bit("SN/Mac写入通过！"), QMessageBox::Ok);
		if (ay.at(4) == 0)
			QMessageBox::critical(nullptr, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("SN/Mac写入不通过"), QMessageBox::Ok);
	}
	if (ay == nullptr)
	{
		QMessageBox::critical(nullptr, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("TCP读取数据错误！"), QMessageBox::Ok);

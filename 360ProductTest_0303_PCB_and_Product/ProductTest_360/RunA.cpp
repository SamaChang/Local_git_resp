...
void RunA::on_pushButton_clicked()
{
	...
	writeCSV(snInf, NULL, "PASS", QString::number(1), currentTime, QString::fromLocal8Bit("开机运行电流"), "A"/*NULL*/, NULL, NULL, QString("%1").arg(curr)/*NULL*/, NULL);
	this->close();
...
void RunA::on_pushButton_2_clicked()
{
	...
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss"); 
	recordResult("FAIL");
	writeCSV(snInf, NULL, "FAIL", QString::number(1), currentTime, QString::fromLocal8Bit("开机运行电流"), "A"/*NULL*/, NULL, NULL, QString("%1").arg(curr), NULL);
	this->close();
...
void RunA::on_pushButton_3_clicked()
{
	...
	if (curr <= 0.5f)
	{
		QMessageBox::information(nullptr, QString::fromLocal8Bit("通过"), QString::fromLocal8Bit("开机运行电流测试通过！"), QMessageBox::Ok);

	}
	else
	{
		QMessageBox::critical(nullptr, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("开机运行电流测试不通过"), QMessageBox::Ok);

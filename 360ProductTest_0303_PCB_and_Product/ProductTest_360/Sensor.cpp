...
{
	ui.setupUi(this);
	this->setWindowTitle(QString::fromLocal8Bit("光感测试"));
...
void Sensor::on_pushButton_clicked()
{
	//saveLog(snInf, QString::fromLocal8Bit("光感测试"), "Pass");
	QDateTime time;
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss");
	writeCSV(snInf, NULL, "PASS", QString::number(10), currentTime, QString::fromLocal8Bit("光感测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(true, 23);
	emit finishedInfo();
}

void Sensor::on_pushButton_2_clicked()
{
	//saveLog(snInf, QString::fromLocal8Bit("光感测试"), "Fail");
	QDateTime time;
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss");
	recordResult("FAIL");
	writeCSV(snInf, NULL, "FAIL", QString::number(10), currentTime, QString::fromLocal8Bit("光感测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(false, 23);
	emit finishedInfo();
}

void Sensor::on_pushButton_3_clicked()
{
	...
	//pack360_head(CMD_SENSOR, payload_len, send_buf);   //加头

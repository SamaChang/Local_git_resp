...
void Uart::on_pushButton_clicked()
{
	...
	writeCSV(snInf, NULL, "PASS", QString::number(6), currentTime, QString::fromLocal8Bit("UART测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(true, 18);
	emit finishedInfo();
}
...
void Uart::on_pushButton_2_clicked()
{
	...
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss");
	recordResult("FAIL");
	writeCSV(snInf, NULL, "FAIL", QString::number(6), currentTime, QString::fromLocal8Bit("UART测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(false, 18);
	emit finishedInfo();
}

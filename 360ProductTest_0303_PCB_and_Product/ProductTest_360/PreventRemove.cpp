...
{
	ui.setupUi(this);
	this->setWindowTitle(QString::fromLocal8Bit("防拆测试"));
...
{
	//saveLog(snInf, QString::fromLocal8Bit("防拆测试"), "Pass");
	//this->close();
	...
	writeCSV(snInf, NULL, "PASS", QString::number(15), currentTime, QString::fromLocal8Bit("防拆测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(true, 31);
	emit finishedInfo();
}
...
{
	//saveLog(snInf, QString::fromLocal8Bit("防拆测试"), "Fail");
	//this->close();
	...
	QString currentTime = time.currentDateTime().toString("yyyy/MM/dd HH:mm:ss");
	recordResult("FAIL");
	writeCSV(snInf, NULL, "FAIL", QString::number(15), currentTime, QString::fromLocal8Bit("防拆测试"), NULL, NULL, NULL, NULL, NULL);
	this->close();
	emit nextTest(false, 31);
	emit finishedInfo();
}
...
void PreventRemove::on_pushButton_3_clicked()
{
	...
	//pack360_head(CMD_PREVENT, payload_len, send_buf);   //加头
	...
	//}
	//if (recv_buf[3] == 1)
	//	QMessageBox::information(nullptr, QString::fromLocal8Bit("通过"), QString::fromLocal8Bit("防拆测试通过！"), QMessageBox::Ok);
	//if (recv_buf[3] == 0)
	//	QMessageBox::critical(nullptr, QString::fromLocal8Bit("错误"), QString::fromLocal8Bit("防拆测试不通过"), QMessageBox::Ok);

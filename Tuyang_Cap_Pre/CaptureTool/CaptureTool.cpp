	...
	,spk1_thread(nullptr), isCircleShoot(false), all_stream_thread(nullptr), isCut(false),multiple(1)
{
	...
	this->setWindowTitle("Capture/Preview_Tool_V1.38");
	...
	connect(ui.action_Test_Depth_IR_SPK, SIGNAL(triggered()), this, SLOT(test_cap_ir_spk_dep()));
...
void CaptureTool::showAllStream()
{
	...
	yuyv.resize(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * 2);   //yuv422都是一个像素2个字节
	yuyv_buffer.resize(UVC_IMG_WIDTH * UVC_IMG_HEIGHT * 2);  //缓存buffer，避免被快速刷掉
	int type = 0x00;
	int type_prev = 0x01;
	...
	while (stopPlay) {
		...
		{
			//cap_ir_spk_dep_condition_ir.notify_one();
		...
		{
			//memcpy(&yuyv_buffer[0], &yuyv[0], UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);
			//cap_ir_spk_dep_condition_dep.notify_one();
			//memcpy(&all_stream_2[0], &yuyv[0], UVC_IMG_HEIGHT*UVC_IMG_WIDTH*2);

			//将深度值改大，可以增强显示(每个像素两个字节，乘以不同大小)
			uint16_t *p = (uint16_t *)&yuyv[0];
			if (val)
			{
				for (int i = 1; i < UVC_IMG_HEIGHT * UVC_IMG_WIDTH; i++)   //yuyv的第一个数据p[0]不可以*深度改变值，会影响0x0b的判断，导致后面存图找不到0x0b
				{
					p[i] *= val;
				}
			}

			memcpy(&yuyv_buffer[0], p, UVC_IMG_HEIGHT*UVC_IMG_WIDTH * 2);
			cap_ir_spk_dep_condition_dep.notify_one();

			//Mat src_depth(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_16UC1, &yuyv[0]);
			Mat src_depth(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_16UC1, p);
			...
			QPixmap pixmap_depth = QPixmap::fromImage(Qtemp);
			//widgets.at(1)->resize(Qtemp.size());
			//widgets.at(1)->show();
			mainThread_show(widgets.at(2), pixmap_depth);
		}
		
		if(yuyv[0]==0x0c)   //SPK
		{
			//cap_ir_spk_dep_condition_spk.notify_one();
			//send_rec("./log.txt", "...", "Receive 0xc");
			memcpy(&all_stream_2[0], &yuyv[0], UVC_IMG_HEIGHT*UVC_IMG_WIDTH);
			cap_ir_spk_dep_condition_spk.notify_one();
			...
			//Sleep(50);
			Mat spk_src(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, /*&all_stream_2[0]*/&all_stream_2[0]);
			//imshow("spk", spk_src);
			//waitKey(0);
			QImage img_spk = QImage((unsigned char*)spk_src.data, spk_src.cols, spk_src.rows, spk_src.step, QImage::Format_Indexed8);  //ir原图转QImage
			QPixmap pixmap_spk = QPixmap::fromImage(img_spk);
			mainThread_show(widgets.at(0), pixmap_spk);
...
{
	uint8_t *ssr = (uint8_t*)malloc(640 * 360 * 2);
	QString filename = "D:\\ARGB\\test_2\\depth0_400x640.raw";
	QString filename_w = "D:\\ARGB\\test_2\\depth0_test_1.raw";
	QFile file(filename);
	QFile file_w(filename_w);
	file.open(QIODevice::ReadOnly);
	file_w.open(QIODevice::WriteOnly);
	QByteArray ss = file.readAll();
	file_w.write(ss);
	int len = file.size();
	memcpy(ssr, ss.data(), 640 * 360 * 2);
	Mat ssrr(640, 360, CV_16UC1, ssr);
	imshow("src", ssrr);
	waitKey(0);

	uint16_t *p = (uint16_t *)ssr;
	for (int i = 0; i < 640 * 360; i++)
	{
		p[i] *= 16;
	}
	Mat dst(640, 360, CV_16UC1, p);
	imshow("dst", dst);
	waitKey(0);
	...
	//	if (yuyv[0] == 0xc)  
	//	{
	//		QString spkName = saveFileName + "/" + "spk" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
	//		QFile fileSpk(spkName);    
	//		if (!fileSpk.open(QIODevice::WriteOnly))
	//		{
	//			QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
	//			return;
	//		}
	//		fileSpk.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //spk
	//		fileSpk.close();
	//		//竖图
	//		Mat spk(640, 400, CV_8UC1, &yuyv[0]);
	//		QString spkPng = saveFileName + "/" + "spk" + QString::number(rgb_ir_dep_pic_id) + ".png";
	//		imwrite(spkPng.toStdString(), spk);
	//	}
	//	//ir
	//	if (yuyv[0] == 0xa)
	//	{
	//		QString irName = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
	//		QFile fileIR(irName);
	//		if (!fileIR.open(QIODevice::WriteOnly))
	//		{
	//			QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
	//			return;
	//		}
	//		fileIR.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //ir
	//		fileIR.close();


	//		//竖图
	//		Mat ir(640, 400, CV_8UC1, &yuyv[0]);
	//		QString irPng = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + ".png";
	//		imwrite(irPng.toStdString(), ir);
	...
	//	if (yuyv[0] == 0xb)  //depth
	//	{
	//		QString depName = saveFileName + "/" + "depth" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
	//		QFile fileDep(depName);
	//		if (!fileDep.open(QIODevice::WriteOnly))
	//		{
	//			QMessageBox::critical(nullptr, QString::fromLocal8Bit("Error"), QString::fromLocal8Bit("Open File error!"), QMessageBox::Ok);
	//			return;
	//		}
	//		fileDep.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);
	//		fileDep.close();

	//		Mat depth(640, 400, CV_16UC1, &yuyv[0]);
	//		QString depthPng = saveFileName + "/" + "Depth" + QString::number(rgb_ir_dep_pic_id) + ".png";
	//		imwrite(depthPng.toStdString(), depth);
	//	}
	//	
...
//0xa,0xb,0xc的顺序是下位机发送图片的顺序，因此要抓取连续的三张图必须以此顺序保存
void CaptureTool::cap_ir_spk_dep_test()
{
//#ifdef 0
//	static uint32_t rgb_ir_dep_pic_id = 0;
//	while (true)
...
//#endif // 1
	...
	//unsigned char *buffer_yuyv_spk = nullptr;
	//unsigned char *buffer_yuyv_ir = nullptr;
	//unsigned char *buffer_yuyv_depth = nullptr;
	static uint32_t rgb_ir_dep_pic_id = 0;
	bool cap = true;
	while (cap)
	{
		if (yuyv[0] == 0xb)
		{
			...
			//	continue;
			//}
			std::unique_lock<std::mutex> lock_spk(m_mutex_spk);
			cap_ir_spk_dep_condition_spk.wait(lock_spk);
			if (yuyv[0] == 0xc)
			{
				//buffer_yuyv_spk = &yuyv[0];
				QString spkName = saveFileName + "/" + "spk" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
				QFile fileSpk(spkName);
				if (!fileSpk.open(QIODevice::WriteOnly))
				{
				...
				//fileSpk.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //spk
				//fileSpk.write((const char *)buffer_yuyv_spk, UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //spk
				fileSpk.write((const char *)&all_stream_2[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //spk
				fileSpk.close();
				//竖图
				//Mat spk(640, 400, CV_8UC1, &yuyv[0]);
				//Mat spk(640, 400, CV_8UC1, buffer_yuyv_spk);
				Mat spk(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, &all_stream_2[0]);
				QString spkPng = saveFileName + "/" + "spk" + QString::number(rgb_ir_dep_pic_id) + ".png";
				imwrite(spkPng.toStdString(), spk);
				...
				std::unique_lock<std::mutex> lock_ir(m_mutex_ir);
				cap_ir_spk_dep_condition_ir.wait(lock_ir);
				if (yuyv[0] == 0xa)
				{
					//buffer_yuyv_ir = &yuyv[0];
					QString irName = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
					QFile fileIR(irName);
					if (!fileIR.open(QIODevice::WriteOnly))
					{
					...
					//fileIR.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //ir
					//fileIR.write((const char *)buffer_yuyv_ir, UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //ir
					fileIR.write((const char *)&all_stream[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT);  //ir
					fileIR.close();


					//竖图
					//Mat ir(640, 400, CV_8UC1, &yuyv[0]);
					//Mat ir(640, 400, CV_8UC1, buffer_yuyv_ir);
					Mat ir(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_8UC1, &all_stream[0]);
					QString irPng = saveFileName + "/" + "IR" + QString::number(rgb_ir_dep_pic_id) + ".png";
					imwrite(irPng.toStdString(), ir);
					...
					//	continue;
					//}
					std::unique_lock<std::mutex> lock_dep(m_mutex_dep);
					cap_ir_spk_dep_condition_dep.wait(lock_dep);
					if (yuyv[0] == 0xb)
					{
						//buffer_yuyv_depth = &yuyv[0];
						QString depName = saveFileName + "/" + "depth" + QString::number(rgb_ir_dep_pic_id) + "_400x640.raw";
						QFile fileDep(depName);
						if (!fileDep.open(QIODevice::WriteOnly))
						{
						...
						//fileDep.write((const char *)&yuyv[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);
						fileDep.write((const char *)&yuyv_buffer[0], UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);
						//fileDep.write((const char *)buffer_yuyv_depth, UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);
						fileDep.close();

						//Mat depth(640, 400, CV_16UC1, &yuyv[0]);
						Mat depth(UVC_IMG_HEIGHT, UVC_IMG_WIDTH, CV_16UC1, &yuyv_buffer[0]);
						//Mat depth(640, 400, CV_16UC1, buffer_yuyv_depth);
						QString depthPng = saveFileName + "/" + "Depth" + QString::number(rgb_ir_dep_pic_id) + ".png";
						imwrite(depthPng.toStdString(), depth);
...
void CaptureTool::test_cap_ir_spk_dep()
{
	//yuyv_buffer.resize(UVC_IMG_WIDTH*UVC_IMG_HEIGHT * 2);
	test_cap_ir_spk_dep_thread = nullptr;
	test_cap_ir_spk_dep_thread = new std::thread(&CaptureTool::cap_ir_spk_dep_test, this);
